...
export default function MatchesPage() {
  ...
  const handleMerge = async (match) => {
    ...
    if (!entity1Id || !entity2Id) {
      setToast({ message: 'Cannot merge: missing entity identifiers in match payload.', type: 'error' });
      return;
    }

    setActionLoadingKey(key);
    try {
      await axios.post(`${apiBaseUrl || 'http://localhost:5000'}/api/merge`, {
        entity1Id,
        entity2Id
      });

      setMatches((prev) => prev.filter((m) => !(getEntityId(m.entity1) === entity1Id && getEntityId(m.entity2) === entity2Id)));
      setToast({ message: 'Merge completed and golden record updated.', type: 'success' });
    } catch (err) {
      const apiError = err?.response?.data?.error;
      setToast({ message: apiError || 'Merge failed. Please try again.', type: 'error' });
    } finally {
